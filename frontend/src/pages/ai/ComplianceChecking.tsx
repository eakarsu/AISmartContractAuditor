...
import { ComplianceCheckResult } from '../../components/AIResultDisplay';
...
export default function ComplianceChecking() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.post('/ai/compliance-check', { sourceCode, standard, contractId: contractId || undefined });
      setResult(res.data);
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><CheckCircle className="w-6 h-6 text-teal-500" /> Compliance Checking</h1>
        <p className="text-gray-500 mt-1">Check smart contracts against ERC standards</p>
      </div>
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <select value={standard} onChange={e => setStandard(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">
              <option value="ERC20">ERC-20</option><option value="ERC721">ERC-721</option><option value="ERC1155">ERC-1155</option>
              <option value="ERC777">ERC-777</option><option value="ERC4626">ERC-4626</option>
            </select>
          </div>
        </div>
        <div className="mb-4">
        ...
        <button onClick={handleSubmit} disabled={loading || !sourceCode.trim()}
          className="flex items-center gap-2 px-6 py-2.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700 disabled:opacity-50 font-medium transition-colors">
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
          {loading ? 'Checking...' : 'Run Compliance Check'}
        </button>
      </div>
      {error && <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">{error}</div>}
      {result && <ComplianceCheckResult data={result} />}
    </div>
  );
}
