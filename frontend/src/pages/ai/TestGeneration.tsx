...
import { TestGenerationResult } from '../../components/AIResultDisplay';
...
export default function TestGeneration() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.post('/ai/test-generation', { sourceCode, language, contractId: contractId || undefined });
      setResult(res.data);
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><TestTube className="w-6 h-6 text-indigo-500" /> Automated Test Generation</h1>
        <p className="text-gray-500 mt-1">AI-generated comprehensive test suites for smart contracts</p>
      </div>
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        ...
        <button onClick={handleSubmit} disabled={loading || !sourceCode.trim()}
          className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 font-medium transition-colors">
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <TestTube className="w-4 h-4" />}
          {loading ? 'Generating...' : 'Generate Tests'}
        </button>
      </div>
      {error && <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">{error}</div>}
      {result && <TestGenerationResult data={result} />}
    </div>
  );
}
