...
import { GasOptimizationResult } from '../../components/AIResultDisplay';
...
export default function GasOptimization() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.post('/ai/gas-optimization', { sourceCode, language, contractId: contractId || undefined });
      setResult(res.data);
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><Fuel className="w-6 h-6 text-amber-500" /> Gas Optimization</h1>
        <p className="text-gray-500 mt-1">AI-powered gas usage optimization suggestions</p>
      </div>
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        ...
        <button onClick={handleSubmit} disabled={loading || !sourceCode.trim()}
          className="flex items-center gap-2 px-6 py-2.5 bg-amber-600 text-white rounded-lg hover:bg-amber-700 disabled:opacity-50 font-medium transition-colors">
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Fuel className="w-4 h-4" />}
          {loading ? 'Analyzing...' : 'Run Gas Optimization'}
        </button>
      </div>
      {error && <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">{error}</div>}
      {result && <GasOptimizationResult data={result} />}
    </div>
  );
}
